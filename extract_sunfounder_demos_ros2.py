#!/usr/bin/env python3
"""
CORRECT VERSION: Using actual PiDog leg dimensions from ROS2.

KEY FIXES from analyzing working ROS2 implementation:
1. LEG = 42mm (not 62mm!)
2. FOOT = 76mm (not 62mm!)
3. Mirror right side legs (YES)
4. Do NOT flip Y coordinate
5. foot_angle = foot_angle - pi/2
"""
...
class SunfounderExtractorCorrectDimensions:
    """Extractor with CORRECT leg dimensions from ROS2."""

    # CORRECT dimensions from ROS2 pidog_ros2/pidog_gaits/inverse_kinematics.py
    LEG = 42.0   # Upper leg length (mm) - NOT 62!
    FOOT = 76.0  # Lower leg length (mm) - NOT 62!

    def __init__(self, standing_height=80):
        """
        Args:
            standing_height: Height in mm (default 80 from ROS2)
        """
        self.standing_height = standing_height
        self.servo_range_low = -np.pi/2
        self.servo_range_high = np.pi
        print(f"\n✓ CORRECT Gait Extractor (from ROS2)")
        print(f"  LEG length: {self.LEG}mm (was 62mm - WRONG!)")
        print(f"  FOOT length: {self.FOOT}mm (was 62mm - WRONG!)")
        print(f"  Standing height: {standing_height}mm")
        print(f"  Mirror right legs: YES")
        print(f"  Flip Y: NO\n")
    ...
    def coord2angle(cls, y, z):
        ...
        Based on ROS2 inverse_kinematics.py coord2angles()
        """
        LEG = cls.LEG
        FOOT = cls.FOOT

        # NO Y FLIP (ROS2 uses coordinates directly)
        ...
        # ROS2: foot_angle = foot_angle - (pi / 2)
        leg_angle = alpha
    ...
    def get_walk_gait(self, fb=1, lr=0):
        """Walking gait from ROS2 walk_gait.py."""
        SECTION_COUNT = 8
        STEP_COUNT = 6
    ...
    def coords_to_actions(self, coords_sequence):
        ...
        Based on ROS2 legs_coords_to_angles()
        """
        actions = []

        for coords in coords_sequence:
            # Gait order: FL, FR, BL, BR (same as ROS2)
            gait_order_angles = []

            for i, (y, z) in enumerate(coords):
                leg_angle, foot_angle = self.coord2angle(y, z)

                # ROS2: Mirror RIGHT legs (odd indices: FR=1, BR=3)
                if i % 2 != 0:
                    leg_angle = -leg_angle
            ...
            # Reorder from gait order [FL, FR, BL, BR] to MuJoCo order [BR, FR, BL, FL]
            # Same as ROS2 controller order
            ...
            # MuJoCo order: BR, FR, BL, FL (matches ROS2 controller order)
            mujoco_angles = [
                BR_hip, BR_knee,
                FR_hip, FR_knee,
                BL_hip, BL_knee,
...
def collect_correct_demonstrations(n_cycles=10, standing_height=80, use_camera=False):
    """Collect demonstrations with CORRECT leg dimensions."""
    print("\n" + "="*70)
    print(" SUNFOUNDER EXTRACTION - CORRECT DIMENSIONS FROM ROS2")
    print("="*70)
    print(f"Cycles: {n_cycles}")
    print(f"Height: {standing_height}mm")
    print("Using ROS2-verified leg dimensions: LEG=42mm, FOOT=76mm")
    print("="*70 + "\n")

    extractor = SunfounderExtractorCorrectDimensions(standing_height=standing_height)
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Sunfounder extraction with CORRECT dimensions from ROS2")
    parser.add_argument(
        "--output-file",
        type=str,
        default="demonstrations/sunfounder_ros2_dims.pkl",
        help="Output file",
    )
    parser.add_argument(
        "--standing-height",
        type=float,
        default=80,
        help="Standing height in mm (default: 80)",
    )
    parser.add_argument(
        "--n-cycles",
        type=int,
