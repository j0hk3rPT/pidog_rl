#!/usr/bin/env python3
"""
FLIPPED FORWARD DIRECTION: Try inverting Y coordinate.

If robot walks backward, it means Sunfounder's coordinate system
has opposite forward direction compared to MuJoCo.

Solution: Negate Y coordinates before IK conversion.
"""
...
class SunfounderExtractorFlipped:
    """Extractor with FLIPPED forward direction."""
    ...
    def __init__(self, standing_height=80):
        ...
        print(f"\n✓ Gait Extractor with FLIPPED forward direction")
        print(f"  Standing height: {standing_height}mm")
        print(f"  FLIPPING Y COORDINATES to fix backward walking\n")
    ...
    def coord2angle(y, z):
        ...
        NOTE: Y coordinate is FLIPPED before calculation.
        """
        LEG = SunfounderExtractorFlipped.LEG
        FOOT = SunfounderExtractorFlipped.FOOT

        # FLIP Y COORDINATE - THIS IS THE KEY FIX
        y = -y
    ...
    def get_walk_gait(self, fb=1, lr=0):
        """Walking gait (same as original Sunfounder)."""
        SECTION_COUNT = 8
        STEP_COUNT = 6
    ...
    def get_trot_gait(self, fb=1, lr=0):
        """Trotting gait (same as original Sunfounder)."""
        SECTION_COUNT = 2
        STEP_COUNT = 3
    ...
    def coords_to_actions(self, coords_sequence):
        """
        Convert coordinates to actions with FLIPPED Y direction.
        """
        actions = []
        ...
        for coords in coords_sequence:
            ...
            for i, (y, z) in enumerate(coords):
                # coord2angle handles the Y flip internally
            ...
            # Normalize to [-1, 1] using environment's range
            normalized_action = []
...
def collect_flipped_demonstrations(n_cycles=10, standing_height=80, use_camera=False):
    """Collect demonstrations with FLIPPED forward direction."""
    print("\n" + "="*70)
    print(" SUNFOUNDER EXTRACTION - FLIPPED FORWARD DIRECTION")
    print("="*70)
    print(f"Cycles: {n_cycles}")
    print(f"Height: {standing_height}mm")
    print("FIX: Negating Y coordinates to correct backward walking")
    print("="*70 + "\n")

    extractor = SunfounderExtractorFlipped(standing_height=standing_height)
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Sunfounder extraction with flipped forward direction")
    parser.add_argument(
        "--output-file",
        type=str,
        default="demonstrations/sunfounder_flipped.pkl",
        help="Output file",
    )
    parser.add_argument(
        "--standing-height",
    ...
    demos = collect_flipped_demonstrations(
