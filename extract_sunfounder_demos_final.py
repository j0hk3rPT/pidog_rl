#!/usr/bin/env python3
"""
FINAL VERSION: No mirroring + Flipped Y coordinate

Combines both fixes:
1. NO MIRRORING - MuJoCo legs are geometrically mirrored in the model
2. FLIPPED Y - Robot base rotation makes +Y map to -X (backward)

This should produce symmetric, forward-walking gaits.
"""
...
class SunfounderExtractorFinal:
    """Extractor with NO MIRRORING + FLIPPED Y."""
    ...
    def __init__(self, standing_height=100):
        """
        Args:
            standing_height: Height in mm (default 100 for better stability)
        """
        self.standing_height = standing_height
        self.servo_range_low = -np.pi/2
        self.servo_range_high = np.pi
        print(f"\n✓ FINAL Gait Extractor")
        print(f"  Standing height: {standing_height}mm")
        print(f"  NO mirroring (MuJoCo handles geometrically)")
        print(f"  FLIPPED Y coordinate (base rotation fix)\n")

    @staticmethod
    def coord2angle(y, z):
        """
        Convert (y, z) coordinates to joint angles.
        Y is FLIPPED to fix backward walking.
        """
        LEG = SunfounderExtractorFinal.LEG
        FOOT = SunfounderExtractorFinal.FOOT

        # FLIP Y COORDINATE - fixes backward walking
        y = -y
    ...
    def coords_to_actions(self, coords_sequence):
        ...
        NO MIRRORING + FLIPPED Y coordinate.
        """
        actions = []
        ...
        for coords in coords_sequence:
            ...
            for i, (y, z) in enumerate(coords):
                # coord2angle handles Y flip internally
                hip_angle, knee_angle = self.coord2angle(y, z)

                # NO MIRRORING - MuJoCo legs are geometrically mirrored
...
def collect_final_demonstrations(n_cycles=10, standing_height=100, use_camera=False):
    """Collect demonstrations with final fixes."""
    print("\n" + "="*70)
    print(" SUNFOUNDER EXTRACTION - FINAL VERSION")
    print("="*70)
    print(f"Cycles: {n_cycles}")
    print(f"Height: {standing_height}mm")
    print("FIX 1: No mirroring (MuJoCo handles geometrically)")
    print("FIX 2: Flipped Y coordinate (base rotation correction)")
    print("="*70 + "\n")

    extractor = SunfounderExtractorFinal(standing_height=standing_height)
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Final Sunfounder extraction")
    parser.add_argument(
        "--output-file",
        type=str,
        default="demonstrations/sunfounder_final.pkl",
        help="Output file",
    )
    parser.add_argument(
        "--standing-height",
    ...
    demos = collect_final_demonstrations(
