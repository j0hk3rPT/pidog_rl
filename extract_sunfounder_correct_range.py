#!/usr/bin/env python3
"""
CORRECT VERSION with proper joint range.

Key fixes based on user verification:
1. Leg naming is CORRECT (no swap needed)
2. Joint range is -π/2 to π/2 (180° total), NOT -π/2 to π
3. LEG=42mm, FOOT=76mm (from ROS2)
4. Mirror right legs (from ROS2)
"""
...
class SunfounderExtractorCorrectRange:
    """Extractor with CORRECT 180° joint range."""
    ...
    def __init__(self, standing_height=80):
        ...
        # CORRECT range: -90° to +90° (180° total)
        self.servo_range_low = -np.pi/2
        self.servo_range_high = np.pi/2  # NOT π!
        print(f"\n✓ CORRECT Joint Range")
        print(f"  LEG: {self.LEG}mm, FOOT: {self.FOOT}mm")
        print(f"  Standing height: {standing_height}mm")
        print(f"  Joint range: {self.servo_range_low:.2f} to {self.servo_range_high:.2f} rad ({degrees(self.servo_range_low):.0f}° to {degrees(self.servo_range_high):.0f}°)")
        print(f"  Total: 180° (was incorrectly using 270°!)\n")
    ...
    def coords_to_actions(self, coords_sequence):
        """Convert coordinates to actions with CORRECT range."""
        actions = []

        for coords in coords_sequence:
            # Gait order: FL, FR, BL, BR (MuJoCo naming is correct!)
            gait_order_angles = []

            for i, (y, z) in enumerate(coords):
                leg_angle, foot_angle = self.coord2angle(y, z)

                # Mirror RIGHT legs (FR=1, BR=3)
                if i % 2 != 0:
                    leg_angle = -leg_angle
            ...
            # MuJoCo actuator order: BR, FR, BL, FL (correct, no swap!)
            mujoco_angles = [
                BR_hip, BR_knee,
                FR_hip, FR_knee,
                BL_hip, BL_knee,
            ...
            # Normalize to [-1, 1] using CORRECT range
            normalized_action = []
...
def collect_demonstrations(n_cycles=10, standing_height=80, use_camera=False):
    """Collect demonstrations with correct joint range."""
    print("\n" + "="*70)
    print(" SUNFOUNDER EXTRACTION - CORRECT 180° JOINT RANGE")
    print("="*70)
    print(f"Cycles: {n_cycles}")
    print(f"Height: {standing_height}mm")
    print("FIX: Using -90° to +90° range (was incorrectly -90° to 180°)")
    print("="*70 + "\n")

    extractor = SunfounderExtractorCorrectRange(standing_height=standing_height)
...
def main():
    ...
    parser.add_argument("--output-file", type=str, default="demonstrations/sunfounder_correct_range.pkl")
