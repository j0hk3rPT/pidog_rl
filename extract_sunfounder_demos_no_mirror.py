#!/usr/bin/env python3
"""
NO MIRRORING VERSION: MuJoCo legs are already geometrically mirrored.

In Sunfounder hardware, servos are physically the same on both sides,
so code manually mirrors right-side angles.

In MuJoCo, left/right legs are geometrically mirrored in the model itself
(different euler rotations), so we should NOT negate the angles!
"""
...
class SunfounderExtractorNoMirror:
    """Extractor WITHOUT right-side mirroring (MuJoCo handles it geometrically)."""
    ...
    def __init__(self, standing_height=80):
        ...
        print(f"\n✓ Gait Extractor WITHOUT mirroring")
        print(f"  Standing height: {standing_height}mm")
        print(f"  MuJoCo legs are geometrically mirrored - no manual negation\n")

    @staticmethod
    def coord2angle(y, z):
        """Convert (y, z) coordinates to joint angles."""
        LEG = SunfounderExtractorNoMirror.LEG
        FOOT = SunfounderExtractorNoMirror.FOOT
    ...
    def coords_to_actions(self, coords_sequence):
        """
        Convert coordinates to actions WITHOUT mirroring.
        MuJoCo model handles geometric mirroring.
        """
        actions = []
        ...
        for coords in coords_sequence:
            ...
            for i, (y, z) in enumerate(coords):
                ...
                # NO MIRRORING - MuJoCo legs are already geometrically mirrored!
...
def collect_demonstrations_no_mirror(n_cycles=10, standing_height=80, use_camera=False):
    """Collect demonstrations without mirroring."""
    print("\n" + "="*70)
    print(" SUNFOUNDER EXTRACTION - NO MIRRORING")
    print("="*70)
    print(f"Cycles: {n_cycles}")
    print(f"Height: {standing_height}mm")
    print("FIX: No manual mirroring (MuJoCo handles it geometrically)")
    print("="*70 + "\n")

    extractor = SunfounderExtractorNoMirror(standing_height=standing_height)
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Sunfounder extraction without mirroring")
    parser.add_argument(
        "--output-file",
        type=str,
        default="demonstrations/sunfounder_no_mirror.pkl",
        help="Output file",
    )
    parser.add_argument(
        "--standing-height",
    ...
    demos = collect_demonstrations_no_mirror(
